...
def path_converter():
    catalog = 'D:/Projects/Blender/render_new_objects_malina/'
    paths = os.listdir(catalog)
    for path in paths:
        char_id = path.split('_')[0]

        # TYPES
        type_seq_to_gif = f'ffmpeg -i {catalog}{path}/r_00%02d.jpg -framerate 25 {catalog}{path}/{char_id}.mp4 -y'
        ...
        # t.start()
        os.system(type_seq_to_gif)
        # t.stop()


def folder_converter():
    mp4_catalog = 'D:/Projects/Blender/mp4/'
    webp_catalog = 'D:/Projects/Blender/webp/'
